  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/3 sm:px-6">
      ...
      <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Task for today:</h3>
      ...
      <div className="max-w-full overflow-x-auto">
        <Table>
          <TableHeader className="border-y border-gray-100 dark:border-gray-800">
            <TableRow>
              {['Task', 'Type', 'Assign By', 'Level', 'Remark', 'Progress', 'Action'].map((header) => (
                <TableCell
                  key={header}
                  isHeader
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
            {data?.data?.map((task) => (
              <TableRow key={task.id}>
                ...
                <TableCell className="py-3 text-gray-500 text-xs dark:text-gray-400">{task.type}</TableCell>
                <TableCell className="py-3 text-gray-500 text-xs dark:text-gray-400">{task.assignedBy}</TableCell>
                ...
                <TableCell className="py-3 text-gray-500 text-xs dark:text-gray-400">{task.status}</TableCell>
                <TableCell className="py-3 text-gray-500 text-xs dark:text-gray-400">
                  <Link to={`/tasks/${task.id}`} className="text-blue-600 hover:underline dark:text-blue-400">
                    Work
                  </Link>
                </TableCell>
              </TableRow>
