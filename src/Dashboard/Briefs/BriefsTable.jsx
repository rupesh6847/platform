...
//   const  [briefs,setBreifs]=useState([])
                  ...
                  <div className="border rounded-2xl border-gray-200 bg-white dark:bg-gray-800 p-6  space-y-4">
                    ...
                    <div className="divide-y divide-gray-100 dark:divide-gray-700">
                      {brief.leadDetails?.map((ld, i) => (
                        <div key={i} className="py-4 space-y-2 border-b dark:border-gray-700">
                          <p className="text-sm text-gray-500">
                            Regions: {ld.regions?.join(', ') || 'N/A'}
                          </p>
                          <p className="text-sm text-gray-500">
                            Audiences: {ld.audiences?.join(', ') || 'N/A'}
                          </p>
                          <p className="text-sm text-gray-500">
                            Job Title: {ld.qualifiers?.JobTitle || 'N/A'}
                          </p>
                          <p className="text-sm text-gray-500">
                            Industry: {ld.qualifiers?.Industry || 'N/A'}
                          </p>
                          <p className="text-sm text-gray-500">
                            Deadline: {ld.deadline || 'N/A'}
                          </p>
                          <p className="text-sm text-gray-500 flex items-center gap-1">
                            <FileText className="h-3 w-3" /> File: {ld.fileAttached || 'N/A'}
                          </p>
                        </div>
                      ))}
                    </div>
...
export default function BriefsTable() {
  ...
  return (
    <>
            ...
            {briefs.map((row) => (
              ...
              >
                ...
                <td className="p-2 text-center">
                  {new Date(row.arrivedOn).toLocaleDateString()}
                </td>
                <td className="p-2 text-center">
                ...
                <td className="p-2 text-center">
                  {row.leadDetails?.[0]?.assignedTo?.join(', ')}
                </td>
                <td className="p-2 text-center">{row.type}</td>
              </tr>
            ))}
      ...
      <BriefDrawer brief={selectedBrief} open={isDrawerOpen} setOpen={setIsDrawerOpen} />
    </>
  );
}
