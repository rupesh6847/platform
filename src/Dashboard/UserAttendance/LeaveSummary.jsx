...
const LeaveSummary = () => {
  ...
  useEffect(() => {
    const fetchLeaveData = async () => {
      try {
        const response = await fetch(`http://localhost:3000/leaves/my-summary/4`);
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusStyles()}`}>
        {status?.toUpperCase() === 'APPROVED' && <CheckCircle2 className="w-3 h-3 inline mr-1" />}
        {status?.toUpperCase() === 'REJECTED' && <XCircle className="w-3 h-3 inline mr-1" />}
        {status || 'Pending'}
      </span>
    );
  };
  ...
  if (loading) {
    return <p className="text-center text-gray-500 py-10">Loading leave summary...</p>;
  }
        ...
        {leaveSummary.map((leave, index) => {
          ...
          return (
            <div key={index} className="border rounded-2xl bg-white p-6">
              ...
              <ProgressBar percentage={percentage} color={leave.color} />
              <p className="text-xs text-gray-500 mt-2">{leave.available} days remaining</p>
            </div>
          );
        })}
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100">
              {leaveHistory.length > 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-5 text-gray-400">
