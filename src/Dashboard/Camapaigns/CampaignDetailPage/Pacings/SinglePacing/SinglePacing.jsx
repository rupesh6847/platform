...
export default function SinglePacing({ pacingId, onBack }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setPhase1Data(res.data || []);
        setRange({ start: 1, end: res.data?.leads?.length || 0 }); // default range
      } catch (e) {
        console.error('Fetch error:', e);
      }
    };
  ...
  // ✅ Corrected Export Logic (matching LeadsTable)
  const handleExport = () => {
    ...
    // ✅ Map leads to flat structure using header key-label pairs
    ...
    // ✅ Clean filename with pacing date
    const formattedDate = pacingDate
      ? pacingDate.slice(0, 10).replace(/:/g, '-')
      : 'date';
            ...
            <div className="bg-white shadow p-3 rounded-lg">
              <div className="flex justify-end items-center gap-2 mb-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-gray-700">Export range:</span>
                  <input
                    type="number"
                    min="1"
                    max={leads.length}
